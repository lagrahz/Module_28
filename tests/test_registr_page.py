...
def test_page_left_registration(selenium):
    # Левая часть формы «Регистрация» содержит логотип и слоган кабинета
    page_reg = RegistrPage(selenium)
    assert page_reg.page_left_registration.text != ''


def test_elements_of_registr(selenium):
    # Блок регистрации формы «Регистрация» содержит основные элементы: поля ввода: Имя, Фамилия, Регион, email, Пароль, Подтверждение пароля; кнопка "Продолжить"
...
def test_names_elements_of_registr(selenium):
    # Названия элементов блока регистрации формы «Регистрация» соответствуют Требованию
    page_reg = RegistrPage(selenium)
...
def test_registr_by_valid_data_(selenium):
    # Регистрация пользователя с невалидными данными: в поле ввода "Имя" и "Фамилия" - знак тире (-)
    page_reg = RegistrPage(selenium)
...
def test_registr_and_redir_auth(selenium):
    # Регистрация пользователя по email, который уже был использован ранее для регистрации и нажатие кнопки 'Войти', для редиректа в форму "Авторизация"
    page_reg = RegistrPage(selenium)
    page_reg.first_name.send_keys(Settings.f_name)
    page_reg.first_name.clear()
    page_reg.last_name.send_keys(Settings.l_name)
...
def test_first_name_by_valid_data(selenium, valid_first_name):
    # Проверка поля ввода "Имя" формы «Регистрация» валидными данными: буквы кириллицы в количестве = 2 ; 3 ; 15 ; 29 ; 30
    page_reg = RegistrPage(selenium)
...
def test_first_name_by_invalid_data(selenium, invalid_first_name):
    # Проверка поля ввода "Имя" формы «Регистрация» невалидными данными: пустое значение; буквы кириллицы в количестве = 1 ; 31 ; 260 ; буквы латиницы; иероглифы; спецсимволы; числа
    page_reg = RegistrPage(selenium)
...
def test_last_name_by_valid_data(selenium, valid_password):
    # Проверка поля ввода "Пароль" формы «Регистрация» валидными данными: символы из букв латиницы прописью и строчные+числа в количестве = 8 ; 15 ; 20
    page_reg = RegistrPage(selenium)
...
def test_passw_registration_confirm_valid_data(selenium):
    # Проверка поля ввода "Пароль" и "Подтвердить пароль" формы «Регистрация» валидными данными (пароли совпадают)
    page_reg = RegistrPage(selenium)
...
def test_passw_registration_confirm_invalid_data(selenium):
    # Проверка поля ввода "Пароль" и "Подтвердить пароль" формы «Регистрация» невалидными данными (пароли не совпадают)
    page_reg = RegistrPage(selenium)
    page_reg.passw_registration.send_keys(Settings.passw1)
    page_reg.passw_registration.clear()
